...
const NumberPattern = ({ number, onPatternChange, isStatic, conflictColumns }) => {
  ...
  const generatePattern = () => {
    ...
    for (let n = 0; n < instances; n++) {
      const space = n === 0 ? baseValue : (baseValue + n) * periodicInterval;
      currentPos += space;
  ...
  const generateGhostPositions = () => {
    let positions = [];
    for (let i = 1; i <= 5; i++) {
      positions.push(baseValue + (baseValue + 1) * i);
    }
    return positions;
  };
  ...
  const handleInstanceDrag = (e, index) => {
    ...
    const handleDrag = (e) => {
      ...
      const newShift = Math.max(-baseValue, startShift + diff);
          ...
          {pattern.map((item, index) => (
            <React.Fragment key={index}>
              ...
              <span 
                ...
                onMouseDown={index === 0 ? handleFirstInstanceDrag : (e) => handleInstanceDrag(e, index)}
              />
            </React.Fragment>
          ))}
        </div>
